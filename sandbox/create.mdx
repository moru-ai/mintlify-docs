...
| `volume_id` / `volumeId` | string | `null` | Volume ID to attach |
| `volume_mount_path` / `volumeMountPath` | string | required with volume | Mount path in sandbox |
...
Attach a persistent volume for data that survives sandbox restarts:
...
# Data in /workspace persists after sandbox is killed
```
...
See [Volumes](/volumes/overview) for more information.
...
<CardGroup cols={2}>
  ...
  <Card title="Volumes" icon="hard-drive" href="/volumes/overview">
    Persistent storage for sandbox data.
  </Card>
</CardGroup>
